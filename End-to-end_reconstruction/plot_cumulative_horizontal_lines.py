...
def read_stalls_from_lines(lines):
    ...
    for i in range(0, len(lines), 2):
        ...
        stalls.append((start_time, end_time))
    return stalls
folder='/home/leonardo/Desktop/ISP_user_collaboration/End-to-end_reconstruction/30_stalls_news_p02_detectorlight/Results_endtoend/mzdfGCdNSHQ_1000Kbps/p_0.2 w_0.1/'
for folder_path in [folder]:
    ...
    y_position_top = max(cumulative_time_candidates[:limit]) + 5  # Y position for the top line
    y_position_bottom = -5  # Y position for the bottom line
    # Plot real stalls
    for i in [0,2,3,4,5,6,7]:
        label = 'actual stall' if i == 0 else ""
        start, end = real_stalls[i]
        plt.plot([start - time_candidate_clean[0], end - time_candidate_clean[0]], [y_position_top, y_position_top], color='red', linewidth=9, label=label)
    ...
    plt.savefig('cumulative_stalldet_horizontal_toy.pdf',bbox_inches='tight')
    plt.savefig('cumulative_stalldet_horizontal_toy.png',bbox_inches='tight')