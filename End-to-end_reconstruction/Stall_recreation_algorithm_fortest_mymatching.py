...
for nr,folder_path in enumerate([news, music,sport]):
    ...
    #MATCHING
    def mismatch_time_length(real_stalls, stall_stoe, tolerance_time, tolerance_length):
        detected_associated = [False] * len(stall_stoe)
        associations = []
        real_lengths = []
        detected_lengths = []
        for real_start, real_end in real_stalls:
            ...
            real_length = real_end - real_start
            for i, (detected_start, detected_end) in enumerate(stall_stoe):
                if abs(real_start - detected_start) <= tolerance_time and detected_end>real_start:
                    detected_associated[i] = True
                    detected = True
                    detected_length = detected_end - detected_start #+p#+ 2*p   text does like that but results are worse
                    if abs(real_length - detected_length)<=tolerance_length:
                        associations.append({
                            # 'real_start': real_start,
                            # 'detected_start': detected_start,
                            # #'real_end': real_end,
                            ...
                            # #'detected_end': detected_end,
                            # 'detected_length': detected_length
                            'distance_start': abs(real_start - detected_start),
                            'distance_length': abs(real_length - detected_length)
                        })
                        real_lengths.append(real_length)
                        detected_lengths.append(detected_length)
                        break
        ...
        # return tp, fn, fp, mae, rmse, associations
    tolerance_time = 1
    tolerance_length = 0.5 #s
    associations,mae,rmse=mismatch_time_length(real_stalls, stall_stoe, tolerance_time, tolerance_length)
    for i in range(len(associations)):
        print(associations[i])
    #np.save('stalls_detection_info'+str(kind)+'.npy',associations)

    print('secure_association',len(associations))
    print('mae',mae)
    print('rmse',rmse)
    ...
    # # 1. Find the max distance_start
    # max_distance_start = max(item['distance_start'] for item in data)
    # # 2. Find the max distance_length
    # max_distance_length = max(item['distance_length'] for item in data)
    # # 3. Get the top 10 elements with the highest distance_start
    # top_10_highest_distance_start = heapq.nlargest(10, data, key=lambda x: x['distance_start'])
    # # Output the results
    # print("Max distance_start:", max_distance_start)
    # print("Max distance_length:", max_distance_length)
    # print("Top 10 elements with highest distance_start:", top_10_highest_distance_start)
    print('###########################################################################')
    print('###########################################################################')
